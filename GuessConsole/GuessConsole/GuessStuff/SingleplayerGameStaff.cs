using GuessConsole.Helpre;
using GuessCore.Helpers;
using GuessCore.Interfaсes;
using System;

namespace GuessConsole.GuessStuff
{
    public class SingleplayerGameStaff : GameStaffBase
    {
        public void Run()
        {
            char exitCh;

            Console.Clear();
            _Guess.Initialize();
            var isBreak = false;
            var interactor = _Guess[InteractorKey.Login];
            ConsoleHelper.GetResponse(interactor, "");
            do
            {
                var req = Console.ReadLine();
                isBreak = ConsoleHelper.GetResponse(interactor, req);
            } while (!isBreak);
            isBreak = false;

            do
            {
                _Guess.Initialize();
                Console.Clear();
                // Конфигурация
                Console.WriteLine(_Guess.GetPlayerState());
                interactor = _Guess[InteractorKey.RespondentAutoConfigure];
                ConsoleHelper.GetResponse(interactor, "");
                do
                {
                    var req = Console.ReadLine();
                    isBreak = ConsoleHelper.GetResponse(interactor, req);
                } while (!isBreak);
                isBreak = false;
                Console.Clear();

                // Процесс игры
                Console.WriteLine(_Guess.GetPlayerState());
                interactor = _Guess[InteractorKey.Guess];
                IRetryCounter retryCounter = _Guess.GetRetryCounter();
                ConsoleHelper.GetResponse(interactor, "");
                do
                {
                    Console.WriteLine(
                        $"Израсходованное число попыток {retryCounter.Attempts}/{retryCounter.NumberOfAttempts}");
                    Console.WriteLine("Введите число:");
                    var req = Console.ReadLine();
                    isBreak = ConsoleHelper.GetResponse(interactor, req);
                    if (!isBreak && !retryCounter.IsAttemptsRemained)
                    {
                        isBreak = true;
                        Console.WriteLine("Вы проиграли.");
                    }
                } while (!isBreak);
                isBreak = false;
                ConsoleHelper.GetResponse(_Guess[InteractorKey.SaveCurentPlayer], "");

                //Продолжить или нет
                do
                {
                    Console.WriteLine();
                    Console.WriteLine("Хотите продолжить? y/n");
                    exitCh = Console.ReadKey().KeyChar;

                    if (exitCh == 'n' || exitCh == 'y')
                    {
                        isBreak = true;
                    }

                } while (!isBreak);
            } while (exitCh == 'y');
        }
    }
}